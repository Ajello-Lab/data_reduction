...
;-
pro gradedark,cube,dark,grade,DISPLAY=display,MAXBAD=maxbad, $
       FILETEST=filetest,ROWTEST=rowtest

   self='gradedark: '
   if badpar(cube,[4,5],3,caller=self+'(cube) ') then return
   if badpar(dark,[4,5],2,caller=self+'(dark) ') then return
   ...
   ;   The superdark is subtracted from each image in the cube before
   ;   taking the average.
   scan=fltarr(ny,nf)
   ...
   if display then begin
      ...
      plot,[0],/nodata,xr=[0,ny-1],yr=[-10,10], $
         xtitle='Row number',ytitle='dark-superdark',title='scan'
      for i=0,nf-1 do $
         oplot,scan[*,i]
