...
;  BINFAC - integer binning factor to use for display.  Default is the
;              smallest binning needed so that the binned image fits on
;              the display, but defaults to 1 for the Z buffer.
...
;  2017/07/13, AMZ, added support for use of Z buffer
;  2019/12/19, MWB, fixed a bug on the overlain plot scale (very subtle)
;  2022/03/14, MWB, added support for !order
;-
pro showsrc,imname,srclist,reflist,OUTFILE=outfile,BINFAC=binfac, $
   ...
   plot,[0],[1],/nodata,xmargin=[0,0],ymargin=[0,0], $
      xr=[0,float(xsm)*sf],yr=yr,xstyle=5,ystyle=5,/noerase
