...
;-
pro obsprop,objcode,in_obs,date1,date2,AMLIMIT=amlimit,OBSFILE=obsfile, $
   ...
   if badpar(in_obs,[2,3,7,8],[0,1],CALLER='OBSPROP: (obs) ',type=codetype) then return
   ...
   if badpar(fixedpos,[0,5,7],1,CALLER='OBSPROP: (FIXEDPOS) ', $
                default=['none'],type=fptype) then return
...
print,'Critical target altitude ',crital*!radeg,' deg'

   ; Compute ephemeris for object
   njd=fix(ceil(jd2-jd1))+1
   jdeph = jd1 + (jd2-jd1)*dindgen(njd)/(njd-1)
   if n_elements(fixedpos) eq 1 then begin
   ...
   print,'Nautical twilight limit for the Sun.'
   ...
   FOR i=0,njd-1 DO BEGIN
      ...
      transam = airmass(jdtrans,ra[i],dec[i],obs.lat,obs.wlon,alt=transalt)
      altoha,crital,dec[i],obs.lat,horzha,type
      if type eq 0 then begin
         jdrise  = jdtrans - horzha/2.0d0/!dpi
         jdset   = jdtrans + horzha/2.0d0/!dpi
      endif
;      obswind,midlst,lat,ra[i],dec[i],jdsrise,jdsset,rtime,rkind, $
;         stime,skind,objtype,AMLIMIT=amlimit
;      obswind,midlst,lat,ra[i],dec[i],jdatrise,jdatset,rtime,rkind, $
;         stime,skind,objtype,AMLIMIT=amlimit
      obswind,midlst,obs.lat,ra[i],dec[i],jdntrise,jdntset,rtime,rkind, $
         stime,skind,objtype,AMLIMIT=amlimit
      if objtype ne -1 and objtype ne 2 then begin
         jdstr,rtime,-12,risestr
         jdstr,stime,-12,setstr
;         prior = (rtime-jdsset)*24.0d0
;         post  = (jdsrise-stime)*24.0d0
