...
pro editmask_eve, event
   ...
   case event_name of
      ...
      'Zoom': begin
         ...
         if x lt 0 or x ge (*state).nx or $
            y lt 0 or y ge (*state).ny then return

         str=strn(x)
         widget_control,(*state).xposid,set_value=str
         str=strn(y)
         widget_control,(*state).yposid,set_value=str
         str=strn((*state).image[x,y])
         widget_control,(*state).dnid,set_value=str
         str=string((*state).mask[x,y],format='(b8.8)')
         widget_control,(*state).maskvalid,set_value=str
         ...
         if (*state).lastx ne x or (*state).lasty ne y then begin
            if x ge 0 and x lt (*state).nx and $
               y ge 0 and y lt (*state).ny then begin
...
pro editmask,image,mask,XSIZE=xsize,YSIZE=ysize,BITNUM=bitnum,STYLE=style, $
   ...
   b2 = widget_base(b1,column=1)
   win1 = widget_draw(b2,XSIZE=szx,YSIZE=szy,RETAIN=2, $
                       /BUTTON_EVENTS,UVALUE='Window')

   b3 = widget_base(b2,/row)
   b4 = widget_base(b3,/column,/grid_layout)
   t=widget_label(b4,value='X')
   t=widget_label(b4,value='Y')
   t=widget_label(b4,value='DN')
   t=widget_label(b4,value='Mask')

   b4 = widget_base(b3,/column,/grid_layout)
   xpos=widget_label(b4,value='',/dynamic_resize,/align_right)
   ypos=widget_label(b4,value='',/dynamic_resize,/align_right)
   dn=widget_label(b4,value='',/dynamic_resize,/align_right)
   maskval=widget_label(b4,value='',/dynamic_resize,/align_right)
