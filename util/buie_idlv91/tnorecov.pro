...
;
;  OBSCODE - Observatory id code, default is '688' (Lowell Observatory), or structure
;
; OUTPUTS:
;
...
pro tnorecov_newobscode, state, newid
   ...
   obsinfo,newid,obs
   if obs.name eq 'unknown' then return

   widget_control,(*state).obscodeid,set_value='('+(*state).obs.obscode+') '
   widget_control,(*state).obsnameid,set_value=(*state).obs.name
...
pro tnorecov_plot, state
   ...
   if (*state).nf gt 0 then begin
      ...
      mam = airmass(curjd,moonra,moondec,(*state).obs.lat,(*state).obs.wlon, $
                    lst=lst,lha=mlha,alt=malt,/hardie)
      sam = airmass(curjd,sunra,sundec,(*state).obs.lat,(*state).obs.wlon, $
      ...
      oppam = airmass(replicate(curjd,npts),oppr,oppd, $
                         (*state).obs.lat,(*state).obs.wlon,lha=oppl,/hardie)
      oppam1 = airmass(replicate(curjd,npts),oppr1,oppd, $
                         (*state).obs.lat,(*state).obs.wlon,lha=oppl1,/hardie)
      oppam2 = airmass(replicate(curjd,npts),oppr2,oppd, $
                         (*state).obs.lat,(*state).obs.wlon,lha=oppl2,/hardie)
      ...
      am = airmass(replicate(curjd,(*state).nf),(*(*state).ra),(*(*state).dec), $
                   (*state).obs.lat,(*state).obs.wlon,lha=lha,alt=alt,az=az,/hardie)
      lha = lha*!radeg/15.0
      alt = alt*!radeg
      az  = az*!radeg
      altoha,(*state).altcrit,(*(*state).dec),(*state).obs.lat,horzha,otype
...
;endif
      ...
      pam = airmass(replicate(curjd,24),pra,0.0,(*state).obs.lat,(*state).obs.wlon,lha=plha)
      ...
      pra = pra/!radeg
      pdec = pdec/!radeg
      pam = airmass(replicate(curjd,240),pra,pdec,(*state).obs.lat,(*state).obs.wlon,lha=plha)
      plha = plha * !radeg / 15.0
...
;------------
pro tnorecov,OBS=in_obs,AMCRIT=amcrit,DT=dt, $
   ...
   self='tnorecon: '
   if badpar(in_obs,[0,1,2,3,7,8],[0,1],caller=self+'(OBS) ', $
                default='688',type=codetype) then return
   if badpar(amcrit,[0,2,3,4,5],0,caller=self+'(AMCRIT) ',default=2.5) then return
   if badpar(dt,[0,2,3,4,5],0,caller=self+'(DT) ',default=0.0d0) then return
   if badpar(minerr1y,[0,2,3,4,5],0,caller=self+'(MIN1YERR) ',default=20.0) then return
   if badpar(db,[0,7],0,caller=self+'(DB) ',default='') then return
   ...
   state = ptr_new({ $
      ...
      obs: obs, $                ; observatory structure information
