...
;  1994/04/05 - Written by Marc W. Buie, Lowell Observatory
;  2019/01/11, MWB, code cleanup for style, no algorithmic chnages
...
pro acre,dirty_im,clean_im,thresh,width, $
   ...
   self='acre: '
   if badpar(dirty_im,[1,2,3,4,5],2,caller=self+'(dirty_im) ', $
                                    dimen=dirtdim) then return
   if badpar(thresh,[1,2,3,4,5],0,caller=self+'(thresh) ') then return
   if badpar(width,[1,2,3,4,5],0,caller=self+'(width) ') then return
   if badpar(exclude,[0,1,2,3,4,5],2,caller=self+'(EXCLUDE) ', $
                                     dimen=exdim) then return
   ...
   if keyword_set(verbose) then $
      print,'  Flattened background: ',avg,' +/- ',stddev, $
            format='(a,f6.3,a,f6.3)'
   ...
   if keyword_set(exclude) then begin
      ...
      mkxyarr,dirtdim[0],dirtdim[1],xarr,yarr
      lastwid = -1
      ...
      for i=0,exdim[1]-1 do begin
         ...
         if keyword_set(verbose) then $
            print,'  Reset ',strtrim(string(count1),2), $
                  ' pixels in mask back to 0 (off).'

         if exclude[4,i] ge 1 then begin

            if keyword_set(verbose) then $
               print,'  New extraction threshold and width, ', $
                     strcompress(exclude[3:4,i])

            if exclude[4,i] ne lastwid then begin

               if keyword_set(verbose) then $
                  print,'  Median smoothing with a width of ', $
                        strtrim(string(exclude[4,i]),2)
