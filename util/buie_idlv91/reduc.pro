...
; 2018/03/13, MWB, non-trivial mods to accomodate new controls on tr rule
;                   
;-
...
function reduc_infoforcing, state,oprule
   forced = [ oprule.ctforce,oprule.k2force,oprule.kforce,oprule.ktforce, $
              oprule.tdforce,oprule.c2force,oprule.x2force, $
              oprule.rcforce,oprule.rxforce]
   z = where(forced eq 1 )
...
pro reduc_opdisplay,state, oprule
   ...
   forcedvalue = [ reduc_fstrn(oprule.ctvec[0],FORMAT="(F10.4)"), $
                   ...
                   '', $
                   oprule.tddate, $
                   reduc_fstrn(oprule.c2vec[0],FORMAT="(F10.4)"), $
                   reduc_fstrn(oprule.x2vec[0],FORMAT="(F10.4)"), $
                   reduc_fstrn(oprule.rcval,FORMAT="(F10.4)"), $
                   reduc_fstrn(oprule.rxval,FORMAT="(F10.4)") $
                   ]
   forcederror = [ reduc_fstrn(oprule.ctvec[1],FORMAT="(F10.4)"), $
                   ...
                   '','', $
                   reduc_fstrn(oprule.c2vec[1],FORMAT="(F10.4)"), $
                   reduc_fstrn(oprule.x2vec[1],FORMAT="(F10.4)"), $
                   '','']
   ...
   ; detail display of the transf entry for tr rules
      ...
      query = [ 'select instrument,rundate,utdate,filter,color1,color2,', $
                'k,k2,kcolor,zeropt,ktime,e_k,e_k2,e_kcolor,e_zeropt,', $
                'e_ktime,jdref,f_k,f_k2,f_kcolor,f_zeropt,f_ktime,nobs,', $
                'chi2,quality,comments', $
                'from '+(*state).transfdbhook[1], $
                'where rundate=' + quote( (*state).rundate), $
                'and instrument=' + quote((*state).inst) ]
...
pro reduc_oplinesjoin,state,opline
   ...
   if oprule.c2force eq 1 then begin
      rulestr[++i]='c2'
      rulestr[++i]=reduc_fstrn(oprule.c2vec[0],FORMAT="(f10.4)")
      rulestr[++i]=reduc_fstrn(oprule.c2vec[1],FORMAT="(f10.4)")
   endif
   if oprule.x2force eq 1 then begin
      rulestr[++i]='x2'
      rulestr[++i]=reduc_fstrn(oprule.x2vec[0],FORMAT="(f10.4)")
      rulestr[++i]=reduc_fstrn(oprule.x2vec[1],FORMAT="(f10.4)")
   endif
   if oprule.rcforce eq 1 then begin
      rulestr[++i]='rc'
      rulestr[++i]=reduc_fstrn(oprule.rcval,FORMAT="(f10.4)")
   endif
   if oprule.rxforce eq 1 then begin
      rulestr[++i]='rx'
      rulestr[++i]=reduc_fstrn(oprule.rxval,FORMAT="(f10.4)")
   endif
   noplast = i
...
pro reduc_oplinesplit, state, opline, LOCALOPRULE=localoprule
   ...
   canoniz = [ $
       ; TR   
            ...
            ; ctforce/vec           k2force/vec     kforce/vec  
                 opt,     na,na,  opt,opt,opt, opt,opt,opt, $
            ;   ktforce  td/date  c2force/vec  x2force/vec
                 opt,opt,     opt,opt  ], $
            ;   rcforce/val  rxforce/val
       ; SL   
   ...
   optargs=['ct','k2','k','kt','td','c2','x2','rc','rx']
   ...
   optarglen= [3,3,3,1,2,3,3,2,2]; arg itself plus vector size
   ...
   tddate:(*state).na_str, $
   c2force:(*state).na_val, $
   c2vec:[float((*state).na_val),float((*state).na_val)], $
   x2force:(*state).na_val, $
   x2vec:[float((*state).na_val),float((*state).na_val)], $
   rcforce:(*state).na_val, $
   rcval:float((*state).na_val), $
   rxforce:(*state).na_val, $
   rxval:float((*state).na_val), $
   n_rulestr:n_rulestr, $
   optargs:optargs, $
   optarglen:optarglen, $
   opttop:0, $
   ...
   if z[0] ge 0 then begin
      ...
      if oprule.valid then begin
         ...
         if rulestr[++i] ne na_str then oprule.c2force=(rulestr[i] eq 'c2')
         if rulestr[++i] ne na_str then oprule.c2vec[0]=float(rulestr[i])
         if rulestr[++i] ne na_str then oprule.c2vec[1]=float(rulestr[i])
         if rulestr[++i] ne na_str then oprule.x2force=(rulestr[i] eq 'x2')
         if rulestr[++i] ne na_str then oprule.x2vec[0]=float(rulestr[i])
         if rulestr[++i] ne na_str then oprule.x2vec[1]=float(rulestr[i])
         if rulestr[++i] ne na_str then oprule.rcforce=(rulestr[i] eq 'rc')
         if rulestr[++i] ne na_str then oprule.rcval=float(rulestr[i])
         if rulestr[++i] ne na_str then oprule.rxforce=(rulestr[i] eq 'rx')
         if rulestr[++i] ne na_str then oprule.rxval=float(rulestr[i])
...
;        refresh window here
      ...
      'FrcAct': begin
         ...
         if  (*state).frcselindex eq  5 then oprule.c2force = event.index eq 1
         if  (*state).frcselindex eq  6 then oprule.x2force = event.index eq 1
         if  (*state).frcselindex eq  7 then oprule.rcforce = event.index eq 1
         if  (*state).frcselindex eq  8 then oprule.rxforce = event.index eq 1
      ...
      'FrcError': begin
         ...
         if  (*state).frcselindex eq  5 then oprule.c2vec[1]=new
         if  (*state).frcselindex eq  6 then oprule.x2vec[1]=new
         (*(*state).oprule) = oprule
         reduc_oplinesjoin,state,opline
         reduc_oplineupdate,state,opline
         reduc_opdisplay,state,oprule
      ...
      'FrcSel': begin
         ...
         ; forced, below, or (*state).forcenm.
          forced = [ oprule.ctforce,oprule.k2force,oprule.kforce, $
                     oprule.ktforce,oprule.tdforce,oprule.c2force, $
                     oprule.x2force,oprule.rcforce,oprule.rxforce]
      ...
      'FrcValue': begin
         ...
         if  (*state).frcselindex eq  5 then oprule.c2vec[0]=new
         if  (*state).frcselindex eq  6 then oprule.x2vec[0]=new
         if  (*state).frcselindex eq  7 then oprule.rcval=new
         if  (*state).frcselindex eq  8 then oprule.rxval=new
         (*(*state).oprule) = oprule
         if not oprule.tdforce or oprule.tddate ne '' then begin
            reduc_oplinesjoin,state,opline
            reduc_oplineupdate,state,opline
...
pro reduc, DIR=dir,BROWSE=browse,INSTDB=instdb, $
   ...
   forcenm = ['CT','K2','K','KT','TD','C2','X2','RC','RX']
   forcedispl = [2,2,2,0,1,2,2,1,1]
   ; if 1 display value, 2 display value and error
