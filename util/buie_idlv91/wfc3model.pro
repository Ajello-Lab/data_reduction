...
; kludge
      if xint-kc lt 0 or xint+kc+1 ge im_xsize or $
         yint-kc lt 0 or yint+kc+1 ge im_ysize then begin
         ; NOP
      endif else begin
   ...
   if jit_rank eq 0 then begin
      if jitter_in gt 0.0 then begin
         ...
         ksz=size(kernel,/dimen)
         if ksz[0] gt im_xsize or ksz[1] gt im_ysize then begin
            print,'kernel is bigger than image stamp, aborting.'
            return
         endif
