...
;     obs    - Observatory code, Marsden's IAUC codes (scalar). (or pre-built structure)
...
;     2024/07/06, MWB, rework for new obs tools, dropped OBSPOS keyword, dropped
;                         support for more than one observatory per call
;-
pro ephem,in_jd,in_obs,code,object,ephemeris,DEBUG=debug,CACHE=cache, $
   ...
   if badpar(in_obs,[1,2,3,7,8],[0,1],caller=self+'(obs) ', $
   ...
   for i=0L,npts-1 do begin
      ...
      if goodcache then begin
         ...
         if debug then begin
            print,jd[i],obs.obscode,code,obj,format='(f16.8,1x,a3,1x,i2,1x,a)'
            if obs.obscode eq 'G**' or obs.obscode eq 'S**' then $
               print,obs.lat,obs.lon,obs.alt,format=fmt
         endif

         printf,pipe,jd[i],obs.obscode,code,obj,format='(f16.8,1x,a3,1x,i2,1x,a)'
         if obs.obscode eq 'G**' or obs.obscode eq 'S**' then $
