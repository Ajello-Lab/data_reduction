...
;-
pro sspos,TIME=time,AMCRIT=amcrit,OBS=in_obs,DEBUG=debug, $
   ...
   if badpar(in_obs,[0,1,2,3,7,8],[0,1],caller='SSPOS: (obs) ', $
                default='G80',type=codetype) then return
...
;print,'start loop'
   while obid ne 'end' do begin
      ...
      fmt='(f13.10,1x,f13.10,1x,f6.1)'

      printf,pipe,curjd, obs.obscode,22+50,obid,format='(f16.8,1x,a3,1x,i2,1x,a)'
      if obs.obscode eq 'G**' or obs.obscode eq 'S**' then $
               printf,pipe,obs.lat,obs.lon,obs.alt,format=fmt
      printf,pipe,prevhr,obs.obscode,22+50,obid,format='(f16.8,1x,a3,1x,i2,1x,a)'
      if obs.obscode eq 'G**' or obs.obscode eq 'S**' then $
               printf,pipe,obs.lat,obs.lon,obs.alt,format=fmt
      printf,pipe,nexthr,obs.obscode,22+50,obid,format='(f16.8,1x,a3,1x,i2,1x,a)'
      if obs.obscode eq 'G**' or obs.obscode eq 'S**' then $
               printf,pipe,obs.lat,obs.lon,obs.alt,format=fmt
      printf,pipe,curjd, obs.obscode,11,   obid,format='(f16.8,1x,a3,1x,i2,1x,a)'
      if obs.obscode eq 'G**' or obs.obscode eq 'S**' then $
               printf,pipe,obs.lat,obs.lon,obs.alt,format=fmt

      if keyword_set(debug) then begin
         print,curjd, obs.obscode,22+50,obid,format='(f16.8,1x,a3,1x,i2,1x,a)'
         if obs.obscode eq 'G**' or obs.obscode eq 'S**' then $
                  print,obs.lat,obs.lon,obs.alt,format=fmt
         print,prevhr,obs.obscode,22+50,obid,format='(f16.8,1x,a3,1x,i2,1x,a)'
         if obs.obscode eq 'G**' or obs.obscode eq 'S**' then $
                  print,obs.lat,obs.lon,obs.alt,format=fmt
         print,nexthr,obs.obscode,22+50,obid,format='(f16.8,1x,a3,1x,i2,1x,a)'
         if obs.obscode eq 'G**' or obs.obscode eq 'S**' then $
                  print,obs.lat,obs.lon,obs.alt,format=fmt
         print,curjd, obs.obscode,11,   obid,format='(f16.8,1x,a3,1x,i2,1x,a)'
         if obs.obscode eq 'G**' or obs.obscode eq 'S**' then $
                  print,obs.lat,obs.lon,obs.alt,format=fmt
      endif
      ...
      if goodobj then begin
         ...
         am  = airmass(replicate(curjd,3),ra,dec,obs.lat,obs.wlon, $
         ...
         transam = airmass(jdtrans,ra[0],dec[0],obs.lat,obs.wlon,alt=transalt)
         rastr,lst,0,lststr,cc

         if h gt -90 then $
            disphase,0.,sun[0],earth[0],phang[0],g,hmag

         altoha,crital,eph[1,0],obs.lat,horzha,type
         ...
         altoha,suncrital/!radeg,sundec,obs.lat,sunhorzha,suntype
         ...
         ssetam  = airmass(jdsset,ra[0],dec[0],obs.lat,obs.wlon,alt=ssetalt)
         sriseam = airmass(jdsrise,ra[0],dec[0],obs.lat,obs.wlon,alt=srisealt)
         ...
         raod=eph[0,0]
         decod=eph[1,0]
         precess,raod,decod,2000.0,ofdate,/radian
         rastr,raod,4,raods,cc
         decstr,decod,3,decods
         ...
         print,'  RA ',raods,'  Dec ',decods,'  ofdate'
         print
