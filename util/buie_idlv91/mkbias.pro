...
;  2018/07/24, MWB, added good return
      ...
      for i=0,nframes-1 do begin
         ...
         if not bad[i] then begin
            ...
            if i eq 0 then begin
               ...
               if extend then $
                  image = float(readfits(ddir+fname[i]+ft,xhdr,exten_no=ix,/silent)) $
               else $
                  image = float(readfits(ddir+fname[i]+ft,xhdr,/silent))
            endelse
...
print,'Stack average ',strn(nf),' images.'
         avgclip,tmpcube,bias,noisemin=rdnoise,success=success
         if success then $
