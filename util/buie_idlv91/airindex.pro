...
function airindex,wave,pressure,temp,relhum
   ...
   n = 64.328d0 + $
        29498.1d0/(146.0d0 - (1.0d0/wave)^2) + 255.4d0/(41.0d0 - (1.0d0/wave)^2)

   pfac = double(pressure) * (1.0d0+(1.049d0-0.0157d0*temp)*1.0d-6*pressure) / $
                                 (720.883d0*(1.0d0+0.003661d0*temp))

   dt   = 100.0d0 - temp
   logp = 2.8808d0 - 5.67d0*dt/(274.1d0+temp-0.15d0*dt)
   f    = (relhum/100.0d0) * 10.0d0^logp

   water = (0.0624d0 - 0.000680d0/wave^2)*f/(1.0d0+0.003661d0*temp)
