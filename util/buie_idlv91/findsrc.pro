...
;            zg = where(xflux gt 0. and yflux gt 0. and fwhmdev lt 2.5,countg)
            zg = where(xflux gt 0. and yflux gt 0. and $
                       fwhmdev lt 5.0 and fwhm gt minvalidfwhm,countg)
         endif else begin
            if not nocrs then begin
               ...
               zg = where(fwhm gt 1.0 and mag lt 30.0 and $
                          ...
                          finite(crsdet)) and snr ge sigthresh and $
                          fwhm gt minvalidfwhm, countg)
            endif else begin
               zg = where(fwhm gt 1.0 and mag lt 30.0 and $
                           snr ge sigthresh and fwhm gt minvalidfwhm, countg)
            endelse
         endelse
         if countg gt 0 and countg ne n_elements(xc) then begin
            xc   = xc[zg]
...
pro findsrc,fn, $
   ...
   if badpar(noinfo,[0,1,2,3],0,caller=self+'(NOINFO) ', $
                default=0) then return
   if badpar(minvalidfwhm,[0,2,3,4,5],0,caller=self+'(MINVALIDFWHM) ', $
                default=-1) then return
