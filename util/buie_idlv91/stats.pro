...
;  2022/02/10, MWB, changed y-axis scaling to always include 0.
;-
pro stats,data,ROBO=robo,SILENT=silent,TITLE=title,NBINS=nbins,XTITLE=xtitle, $
            ...
            GAUSSIAN=gaussian,BACKGROUND=background,COLOR=color,MODE=mode
   ...
   if badpar(background,[0,2,3],0,CALLER=self+'(BACKGROUND) ',DEFAULT='ffffff'xl) then return
   if badpar(color,[0,2,3],0,CALLER=self+'(COLOR) ',DEFAULT=0) then return
   ...
   xyouts,0.1*nbins*binsz+min_data,yr[0]+0.9*(yr[1]-yr[0]), $
      'Mean '+string(avg),/data,align=0.0,color=color,charsize=charsize
   xyouts,0.1*nbins*binsz+min_data,yr[0]+0.8*(yr[1]-yr[0]), $
      'sigma '+string(stddev),/data,align=0.0,color=color,charsize=charsize
