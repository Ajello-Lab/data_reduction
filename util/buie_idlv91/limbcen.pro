...
pro limbcen,image,xcen,ycen,DISPLAY=display
   ...
   self='limbcen: '
   if badpar(image,[1,2,3,4,5],[2,3],CALLER=self+'(image) ', $
                                     DIMEN=dimen,RANK=rank) then return
   if badpar(display,[0,1,2,3,4,5],0,CALLER=self+'(display) ', $
   ...
   mkxyarr,dimen[0],dimen[1],xarr,yarr
   ...
   for i=0,nim-1 do begin
      ...
      if count eq 0 then begin
         print,self+'abort, no limb points found',i
         return
      endif
