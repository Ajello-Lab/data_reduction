...
;   a ''.
;
;   This is also useful for strings passing to shell scripts.
; CATEGORY:
;   Database
; CALLING SEQUENCE:
;   result = quote(string)
...
;  DELIMITER - Character (string, really) to wrap around start and end of
;               the output string.  The default is a single quote (').  Set
;               this to something else if needed.  Note that anything other
;               than the default will break the use of dequote to undo this
...
;  2022/11/26, MWB, added DELIMITER keyword, added ( and ) to protect
;-
function quote,str,LITERAL=literal,DELIMITER=delimiter

   self='QUOTE: '
   if badpar(str,7, [0,1], caller=self+'(str) ',RANK =strrank) then return,''
   if badpar(literal,[0,1,2,3], 0, caller=self+'(LITERAL) ', $
             default=0) then return,''
   if badpar(delimiter,[0,7], 0, caller=self+'(DELIMITER) ', $
             default="'") then return,''
   ...
   if sizeable[0] ge 0 then begin
      strippable = where( strmid(newstr[sizeable],0,1) eq tdel  and $
                          strmid(newstr[sizeable],0,/REVERSE_OFFSET) eq tdel)
   ...
   newstr = tdel + newstr + tdel   ; wrap in single quotes
   return,newstr
end
