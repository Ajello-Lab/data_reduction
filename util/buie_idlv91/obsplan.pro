...
;  2022/08/09, MWB, fixed support for obs input types
;-
pro obsplan,in_objcode,in_obs,in_date,OBSFILE=obsfile,AMLIMIT=amlimit, $
   ...
   if badpar(in_obs,[2,3,7,8],[0,1],CALLER='OBSPLAN: (obs) ',type=codetype) then return
   ...
   for i=0,n_elements(objcode)-1 do begin
      ...
      am = airmass(jdn,ra,dec,obs.lat,obs.wlon,alt=alt,lha=lha,lst=lst)
   ...
   xyouts,0.05,0.02,obs.name,align=0.0,/normal
