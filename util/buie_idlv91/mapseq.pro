...
;  1997/06/21, MWB, undocumented changes
;  2023/02/27, MWB, cosmetic editing and slight changes to graphical output
;-
pro mapseq,root,frame1,frame2,scale,step=step
  ...
  for i=frame1,frame2,step do begin
     setwin,1
     erase
     tvmaps,root+string(i,format='(i4.4)'),scale,pl,ch,pldef,chdef,win=1
     ...
     w1=tvrd(true=3)
     setwin,0,xsize=width,ysize=height/2
     erase
     !p.multi=[2,1,2]
     plot,w,plhist,xmargin=[5,1],ymargin=[2,1]
     oplot,w,pldhis*max(plhist)/max(pldhis),color=!d.n_colors/2
     oplot,w,plhist
     plot,w,chhist,xmargin=[5,1],ymargin=[2,1]
     oplot,w,chdhis*max(chhist)/max(chdhis),color=!d.n_colors/2
     oplot,w,chhist
     xyouts,12*scale,height/2-12*scale,string(i,format='(i4.4)'),/device,charsize=scale
     w0=tvrd(true=3)
     setwin,2,xsize=width,ysize=height
     erase
     tv,w1,0,height/2,true=3
     tv,w0,0,0,true=3
     xinteranimate,frame=j,window=2
     j = j+1
  endfor
