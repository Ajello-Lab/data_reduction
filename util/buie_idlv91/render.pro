...
;  HAP3  - 3rd generate Hapke scattering model, 2012 and the book,
;            "Theory of Reflectance and Emittance Spectroscopy (second edition)"
;            This uses bidr4.pro.  This parameter must be a structure to
;            provide the ancillary parameters needed by bidr4.  It must have
;            the following tags:
;                hs - Shadow hiding (SHOE) compaction parameter
;                hc - Coherent backscatter (CBOE) compaction parameter
;                pfn - Name of the phase function (procedure) to use.
;                        '' means that the phase function is already
;                            evaluated and the values given are these
;                            values.
;                pval - arguments for the phase function (or the evauluated
;                         phase function if pfn='')
...
;   2023/08/11, MWB, added HAP3 option
;-

pro render,map,radius,in_scale,in_pole,selat,selon,sslat,sslon,nx,ny,image, $
      LIN=xlimb,MIN=k,HAPKE=hapke,HAP2=hapke2,HAP3=hapke3,H93=h93, $
   ...
   IF fullcompute THEN BEGIN
      ...
      if count_zon gt 0 then begin
         ...
         if ill_count gt 0 then begin
            ...
            ; Hapke scattering function, third generation.
            endif else if keyword_set(hapke3) then begin
               if not silent then print,'Hapke 2012 theory used.'
               if hapke3.pfn eq '' then begin
print,'A'
                  image[zon[ill]] = bidr4(map[xidx[ill],yidx[ill]], $
                                          mu[ill],smu[ill],g[ill], $
                                          hapke3.hs,hapke3.hc,hapke3.pval, $
                                          hapke3.b0s,hapke3.b0c, $
                                          hapke3.theta)*!pi
               endif else begin
                  pparms=n_elements(hapke3.pval)
print,'B'
                  image[zon[ill]] = bidr4(map[xidx[ill],yidx[ill]], $
                                          mu[ill],smu[ill],g[ill], $
                                          hapke3.hs,hapke3.hc,hapke3.pval, $
                                          hapke3.b0s,hapke3.b0c,hapke3.theta, $
                                          pfn=hapke3.pfn,pparms=pparms)*!pi
               endelse
...
print,'E'
            image[geom[zon*illc:(zon+1)*illc-1]] = $
               bidr4(map[geom[xidx*illc:(xidx+1)*illc-1], $
                         geom[yidx*illc:(yidx+1)*illc-1]], $
                     geom[mu*illc:(mu+1)*illc-1], $
                     geom[smu*illc:(smu+1)*illc-1], $
                     geom[g*illc:(g+1)*illc-1], $
