...
pro lcfit,lon,in_phang,data,sig,nterms,c,csig, $
   ...
   if badpar(dymf,[0,1,2,3,4,5,6],0,caller=self+'(DYMF) ', $
   ...
   yfit = curvefit(double(lon),double(data),double(w),guess,sigterms, $
                   function_name="LCFUN")
   ...
   if not nodisplay then begin
      ...
      ploterror,lon,data_1,sig,psym=4, $
         xr=lonran,yr=mean(minmax(data_1))+dym/dymf, $
         xtit='East longitude (degrees)',ytit='Phase corrected magnitude', $
         tit=title
      oplot,synlon,syn,color=cpalette(1)
      ...
      ploterror,in_phang,data_2,sig,psym=4, $
         ...
         tit=title
      oplot,[0],[c[1]],psym=5,color=cpalette(1)
      oplot,synphase,synp,color=cpalette(1)

      setwin,2,xsize=540,ysize=320
      ploterror,lon,data-yfit,sig,psym=4,xr=lonran, $
         yr=mean(minmax(data-yfit))+dym, $
