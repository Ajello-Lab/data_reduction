...
;                Gaia star catalog.
;                   an external program, grefnet, is called to get a sub-catalog
;                   This requires setting the environment variable
;                      GAIA_CAT
;                   whose value is the name of a directory where my compressed
;                   binary catalog files can be found.
;
...
;  2018/05/11, MWB, Changed to support Gaia DR2 catalog
;  2018/06/15, MWB, fixed a case that crashed due to a missing variable
;-
...
pro astrom,root,fileno, $
   ...
   if badpar(twomass,[0,7],0,CALLER='ASTROM: (TWOMASS) ', $
                              default='') then return
   if badpar(usnocat,[0,7],0,CALLER='ASTROM: (USNOCAT) ', $
                              default=0) then return
...
;         psize=fix(sqrt(sz[1]^2+sz[2]^2)*pscale0*1.05+0.5+catpad)
      ...
      if not exists(starfile) or newcat then begin
         killrefstars=1
         if usnocat then begin
            print,'       --> running refnet....'
            gepoch=0.
         endif else begin
            jd2year,hdrinfo.jd,gepoch
            print,'       --> running refnet....  epoch=', $
                  strn(gepoch,format='(f7.2)')
         endelse
         refnet,objra,objdec,psize,psize,maglim,maglim, $
            starfile,TWOMASS=twomass,gaia=gepoch
         goodsolution = 0
      endif else begin
         killrefstars=0
         jd2year,hdrinfo.jd,gepoch
...
;               oplot,astx,asty,psym=8,color=color1,symsize=0.2
         ...
         if countok gt 0 then begin
            ...
            if not nodisplay then begin
               setwin,15
               if twomass eq '' and gepoch eq 0. then begin
                  xtitle='USNO catalog red magnitude'
               endif else if twomass eq '' then begin
                  xtitle='Gaia G magnitude'
               endif else begin
