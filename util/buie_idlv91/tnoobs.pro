...
;               mostly for testing.
;
;  OBS - Observatory id code, default is '688' (Lowell Observatory), or structure
;
; OUTPUTS:
;
...
pro tnoobs_plot, state
   ...
   if (*state).nf gt 0 then begin
      ...
      oppam = airmass(replicate(curjd,npts),oppr,oppd,(*state).obs.lat,(*state).obs.wlon,lha=oppl,/hardie)
      oppam1 = airmass(replicate(curjd,npts),oppr1,oppd,(*state).obs.lat,(*state).obs.wlon,lha=oppl1,/hardie)
      oppam2 = airmass(replicate(curjd,npts),oppr2,oppd,(*state).obs.lat,(*state).obs.wlon,lha=oppl2,/hardie)
      ...
      am2= airmass(replicate(curjd+(*state).patlen/24.0,(*state).nf),(*(*state).ra),(*(*state).dec), $
                   (*state).obs.lat,(*state).obs.wlon,/hardie)
      lha = lha*!radeg/15.0
      alt = alt*!radeg
      az  = az*!radeg
...
pro tnoobs,OBS=in_obs,AMCRIT=amcrit,DT=dt
   ...
   if badpar(in_obs,[0,1,2,3,7,8],[0,1],caller='TNOOBS: (OBSCODE) ', $
