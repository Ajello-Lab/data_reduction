...
;  Fit a numerical PSF to one or more sources in an image, fixed position.
; DESCRIPTION:
...
;  CCD data processing
; CALLING SEQUENCE:
;  psffit2,image,psf,region,xin,yin,counts
; INPUTS:
...
;  xin    - X location for source(s)
;  yin    - Y location for source(s)
...
;  2017/06/09, written by Marc W. Buie, Southwest Research Institute
;     cloned from psffit.pro
;-
function psffit2_func,vals
   common psffit2_com,info

   nsrc=n_elements(vals)
   f=vals > 0

   model=fltarr(info.nx,info.ny)
   addpsf,model,info.x,info.y,f,info.psf,psfmax=[info.xm,info.ym]
...
;   chisq = total((dimage/sigarr)^2) / info.npix
   ...
   if chisq lt info.chibest then begin
      if not info.silent then print,info.chicount,chisq,f[0]
      info.chibest = chisq
   endif
...
pro psffit2,image,psf,region,xin,yin,counts, $
   ...
   if badpar(xin,[2,3,4,5],[0,1],caller=self+'(xin) ',npts=npts) then return
   if badpar(yin,[2,3,4,5],[0,1],caller=self+'(yin) ') then return
   ...
   if min(psfmax) lt 0 then begin
      if not silent then print,'measure PSF properties'
      boxm,psf,psfdim[0]/2,psfdim[1]/2,psfdim[0]/2-1,psfdim[1]/2-1,xm,ym
      findmax,xm,ym,psf,xmax,ymax,fm
      psfmax=[xmax,ymax]
   endif else begin
      if not silent then print,'PSF properties given'
      xmax=psfmax[0]
      ymax=psfmax[1]
   endelse
   ...
   ; positive values define an annulus for local sky
      ...
      endelse
   endelse
   if not silent then print,'sky ',meansky,skyerr
   if not silent then print,'gain, rdnoise',gain,rdnoise
   ...
   if x0 ge 0 and x0 lt sz[0] and x1 ge 0 and x1 lt sz[0] and $
      ...
      ftol=1.0e-4
      fitval=amoeba(ftol,FUNCTION_NAME='psffit2_func', $
                         P0=start, SCALE=scale, NMAX=nmax)

      if n_elements(fitval) ge 1 then begin
         chisq=psffit2_func(fitval)
         counts=trimrank(fitval)
      endif else begin
         counts=fltarr(npts)
         chisq=-1.0
      endelse
