...
;  2022/05/28, MWB, added save of image to an unannotated FITS file with
;                     wcs information included.
;-
   ...
   case event_name of
      ...
      'THE_MENU': begin
         case event.value of
            ...
            'Reload Configuration': begin
               if (*state).fnconfig ne '' then begin
                  finder_loadconfig,(*state).fnconfig,valid,state
                  if valid then finder_refresh,state,2
               endif
            end
            ...
            'Save as FITS': begin
               ...
               rastr,(*state).racen,1,ras
               decstr,(*state).deccen,0,decs
               mkhdr,hdr,(*(*state).psynimage)
               sxaddpar,hdr,'RA',ras
               sxaddpar,hdr,'DEC',decs
               sxaddpar,hdr,'DATE-OBS',jds
               astinfo,hdr,(*(*state).psyninfo),/toheader
               fn=strtrim(strcompress(fn),2)
               fn=repchar(fn,'/','')
               fn=repchar(fn,':','')
               fn=repchar(fn,' ','_')+'.fits'
               print,'Saving image to ',fn
               writefits,fn,(*(*state).psynimage),hdr
            end
