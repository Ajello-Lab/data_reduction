...
pro futureob,start,length,obs,object,FULLNAME=fullname,DT=dt
   ...
   euler,ra_r,dec_r,ecllon,ecllat,3,/radian
   euler,ra_r,dec_r,gallon,gallat,1,/radian
   ...
   z = where(dm eq min(dm))
   jdstr,jd[z],0,str
   xyouts,0.2,0.9,'Brightest at '+str+' is '+strn(min(dm),format='(f7.2)'),/normal
   ...
   setwin,3
   !p.multi=[0,1,4]
   plot,yr,ecllat*!radeg,chars=cs,ytitle='Ecliptic Latitude'
   if goodz gt 0 then $
      for i=0,n_elements(i1)-1 do $
         oplot,yr[i1[i]:i2[i]],ecllat[i1[i]:i2[i]]*!radeg,color=color,thick=th
   plot,yr,ecllon*!radeg,chars=cs,ytitle='Ecliptic Longitude'
   if goodz gt 0 then $
      for i=0,n_elements(i1)-1 do $
         oplot,yr[i1[i]:i2[i]],ecllon[i1[i]:i2[i]]*!radeg,color=color,thick=th
   plot,yr,gallat*!radeg,chars=cs,ytitle='Galactic Latitude'
   if goodz gt 0 then $
      for i=0,n_elements(i1)-1 do $
         oplot,yr[i1[i]:i2[i]],gallat[i1[i]:i2[i]]*!radeg,color=color,thick=th
   plot,yr,gallon*!radeg,chars=cs,ytitle='Galactic Longitude'
   if goodz gt 0 then $
      for i=0,n_elements(i1)-1 do $
         oplot,yr[i1[i]:i2[i]],gallon[i1[i]:i2[i]]*!radeg,color=color,thick=th
