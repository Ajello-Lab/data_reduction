...
;  OBSCODE - Observatory code, default = '688', Lowell Observatory, or structure
;
; OUTPUTS:
;
...
pro fieldobs_newobscode, state, newid
   ...
   if (*state).obscode eq 'man' then begin
      obsinfo,'G**',obs, $
            40.252802d0,105.162537d0,1574.0,'Dunrhomin',/degrees,/west
   endif
...
pro fieldobs_plot, state
   ...
   if (*state).nf gt 0 then begin
      ...
      ; daytime
         ...
         if dayleft lt 0.0 then dayleft += 24.0
         rastr,dayleft*15.0/!radeg,-2,timeleftstr
...
pro fieldobs_updatesel, state, newsel
   ...
   if (*state).select lt 0 then begin
      ...
      altoha,(*state).altcrit,dec,(*state).obs.lat,horzha,type
...
pro fieldobs,file,OBS=in_obs,AMCRIT=amcrit,DT=dt
   ...
   if badpar(in_obs,[0,2,3,7,8],[0,1],caller=self+'(OBS) ', $
   ...
   endelse
   if obs.name eq 'unknown' then begin
      print,'Observatory code ',in_obs,' invalid.'
      return
   endif
   ...
   state = ptr_new({ $
      ...
      obs: obs, $                ; Observatory information structure
