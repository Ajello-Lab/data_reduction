...
pro ajello_lab_regression_example ; , file, model_fit_arr3, spec_cal_norm, wave_spec, spec_norm, lbh_norm
  compile_opt idl2
  ...
  file_name = 'Calibrated_' + file_basename(file_data, '.idl')
  ...
  ; model_fit_arr1 = fltarr(num_wave_spec, num_feat)
  ; for i = 0, num_feat - 1 do $
  ; model_fit_arr1[*, i] = modeli[*, i] * param[i]

  ; spec_fit1 = total(model_fit_arr1, 2) + const
  ...
  ; p3 = plot(wave_spec, spec_fit1, color = 'red', /over)
  ...
  ; p2 = plot(wave_spec, model_fit_arr1[*, 0], /over, color = 'red', thick = thick)
  ; p3 = plot(wave_spec, model_fit_arr1[*, 1], /over, color = 'orange', thick = thick)
  ; p4 = plot(wave_spec, model_fit_arr1[*, 2], /over, color = 'yellow', thick = thick)
  ; p5 = plot(wave_spec, model_fit_arr1[*, 3], /over, color = 'green', thick = thick)
  ; p6 = plot(wave_spec, model_fit_arr1[*, 4], /over, color = 'blue', thick = thick)
  ; p7 = plot(wave_spec, model_fit_arr1[*, 5], /over, color = 'indigo', thick = thick)
  ; p8 = plot(wave_spec, model_fit_arr1[*, 6], /over, color = 'violet', thick = thick)
  ; for i = 0, num_atomic - 1 do $
  ; pi = plot(wave_spec, model_fit_arr1[*, 7 + i], /over, color = 'red', thick = thick)
  ...
  win = window(dim = [1200, 800])
  thick = 2
  p1 = plot(wave_spec, spec_cal_norm, current = win, thick = thick, xr = [110, 185], layout = [1, 2, 1], title = file_name, $
    xtitle = 'Wavelength (nm)', ytitle = 'Intensity (arb. units)')
