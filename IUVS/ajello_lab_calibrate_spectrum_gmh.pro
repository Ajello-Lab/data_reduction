...
;
pro ajello_lab_calibrate_spectrum_gmh, arr, $
  wave_spec, $
  spec_cal, $
  ...
  if n_params() eq 0 then begin
    ...
    ndx = where( file_names.contains('30EV') and file_names.contains('TEST19'), count_file )
    if count_file gt 1 then begin
      print, 'more than one data file identified'
      stop
    endif
    
    file_data = file_names[ndx[0]]
  ...
  ny = sz[1]
  pix = findgen(nx)
  ndx_left = where( pix lt 0.05*nx )
  ndx_right = where( pix gt 0.95*nx )
  ;spec_left = mean(spec[0 : long(0.1 * n_elements(spec))])
  ;spec_right = mean(spec[long(0.9 * n_elements(spec)) : n_elements(spec) - 1])
  spec_left = mean(spec[ndx_left])
  spec_right = mean(spec[ndx_right])
  ...
  if keyword_set(show_plots) then begin
    ...
    ps1 = plot_shade( p1, wave_spec[ndx_left[0]], wave_spec[ndx_left[-1]], fill_transparency=80 )
    ps2 = plot_shade( p1, wave_spec[ndx_right[0]], wave_spec[ndx_right[-1]], fill_transparency=80 )
    markerp,p1,y=spec_left,linestyle=2,color='blue'
    markerp,p1,y=spec_right,linestyle=2,color='red'
