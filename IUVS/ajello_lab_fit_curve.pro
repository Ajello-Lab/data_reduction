...
;
pro ajello_lab_fit_curve
  compile_opt idl2

  data_path = '/Users/benjamincondit/Desktop/Final Curve_Averaged_Values.csv'
  save_path = '/Users/benjamincondit/Desktop/'

  readcol, data_path, x_out, y_out, err_out, delimiter = ',', format = '(F10.2, F10.6, F10.8)', /silent

  ; ##### Polynomial Fit #####
  xvals = double(indgen(70)) + 115
  co = poly_fit(double(x_out), double(y_out), 6, measure_errors = err_out, /double)
  fit_curve = co[0] + co[1] * xvals + co[2] * xvals ^ 2 $
    + co[3] * xvals ^ 3 + co[4] * xvals ^ 4 + co[5] * xvals ^ 5 + co[6] * xvals ^ 6
  print, co

  ; ##### Combined Exponential Decay and Polynomial Fit #####
  ...
  ; ##### Lowess Fit #####
  ; lowess, x_out, y_out, 3, fit_curve
  ; xvals = x_out
