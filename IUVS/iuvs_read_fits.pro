...
;:Private:
;-
FUNCTION irf_REPCHR, OLD, C1, C2, help=hlp
...
;:Private:
;-
pro irf_fits_info, filename, SILENT=silent,TEXTOUT=textout, N_ext=n_ext, extname=extname
 On_error,2
 compile_opt idl2
 ...
 if N_params() lt 1 then begin
  ...
  if n_elements(filename) eq 0 then message, 'No files specified'
  if filename eq '' then message,'Blank filename specified'
...
; File is gzip compressed if it ends in .gz or .ftz 
   ...
   if ~silent then begin 
    ...
    irf_textopen, 'FITS_INFO', TEXTOUT=textout
   endif
...
;                               Get values of BITPIX, NAXIS etc.
   ...
   ELSE: begin 
         message, ' Illegal value of BITPIX = ' + irf_strn(bitpix) + $
         ' in header',/CON
         goto, SKIP
         end
   endcase
  ...
  if Naxis GT 0 then begin
         descript = descript + ' ' + irf_strn(IDL_type)
         Nax = irf_sxpar( hd, 'NAXIS*')
         if N_elements(Nax) LT Naxis then begin 
              message, $
                 'ERROR - Missing required NAXISi keyword in FITS header',/CON
                  goto, SKIP
         endif
         for i = 1, Naxis do descript = descript + ' '+irf_strn(Nax[i-1])
  endif

  end_rec = where( strtrim(strmid(hd,0,8),2) EQ  'END')

  exname = irf_sxpar(hd, 'extname', Count = N_extname)
...
;  Read header records, till end of header is reached
  ...
  while (end_rec[0] EQ -1) && (~eof(lun1) ) do begin
       ...
       if get_extname then begin
           exname = irf_sxpar(hd1, 'extname', Count = N_extname)
 ...
 descript = irf_strn( n_hdrec ) + descript
...
; Check if all headers have been read 
 ...
 if ( simple EQ 0 ) && ( strlen(irf_strn(exten)) EQ 1) then goto, END_OF_FILE  
...
; Skip the headers and data records
    ...
    ptr = ptr + nrec*2880L
    if compress[nf] then irf_MRD_skip,lun1,nrec*2880L else point_lun,lun1,ptr
...
;
 ...
 if ~SILENT then begin
 printf,!textunit,file,' has ',irf_strn(N_ext),' extensions'
 printf,!textunit,'Primary header: ',irf_gettok(fdescript,' '),' records'

 Naxis = irf_gettok( fdescript,' ' ) 

 If Naxis NE '0' then begin

 case irf_gettok(fdescript,' ') of
 ...
 for i = 0,fix(Naxis)-1 do image_desc = image_desc + ' '+ irf_gettok(fdescript,' ')
 ...
 if N_ext GT 0 then begin
  ...
  printf, !TEXTUNIT, 'Extension ' + irf_strn(i) + ' -- '+extname[i]

  header_desc = '               Header : '+irf_gettok(fdescript,' ')+' records'
  printf, !textunit, format = '(a)',header_desc

  table_type = irf_gettok(fdescript,' ')
  ...
  table_dim = fix( irf_gettok( fdescript,' ') )
  if table_dim GT 0 then begin
          table_type = irf_gettok(fdescript,' ')
          for j = 0, table_dim-1 do $
                table_desc = table_desc + irf_gettok(fdescript,' ') + ' '
  endif
  table_desc = table_desc + ')'
  ...
  if ~silent then irf_textclose, TEXTOUT=textout
  return
...
;:Private:
;-
FUNCTION irf_FXMOVE, UNIT, EXTEN, SILENT = Silent, EXT_NO = ext_no, ERRMSG=errmsg
...
; 
	...
	'EOF encountered while moving to specified extension'
		...
		ENDIF
                ...
                irf_MRD_HREAD, UNIT, HEADER, STATUS, SILENT = Silent, $
		...
		ENDIF    
		       ...
		       EXTNAME = STRTRIM(irf_SXPAR(HEADER,'EXTNAME',COUNT=N_name),2)
			 if N_NAME GT 0 THEN $
			...
			ENDIF	
		ENDIF	         
                BITPIX = irf_FXPAR(HEADER,'BITPIX')
                NAXIS  = irf_FXPAR(HEADER,'NAXIS')
                GCOUNT = irf_FXPAR(HEADER,'GCOUNT') 
                IF GCOUNT EQ 0 THEN GCOUNT = 1
                PCOUNT = irf_FXPAR(HEADER,'PCOUNT')
                
                IF NAXIS GT 0 THEN BEGIN 
                        DIMS = irf_FXPAR(HEADER,'NAXIS*')           ;Read dimensions
...
;:Private:
;-
FUNCTION irf_FXPOSIT, XFILE, EXT_NO, readonly=readonly, COMPRESS=COMPRESS, $
...
; the bidirectional pipe. 	
			...
			unit = irf_fxposit( file, ext_no, /no_fpack,extnum=extnum)
			free_lun,unit
			exten = extnum
			endif 
...
;For Uncompresed files test that the first 8 characters are 'SIMPLE'
	...
	stat = irf_FXMOVE(unit, exten, SILENT = Silent, EXT_NO = extnum, $
	ERRMSG=errmsg)
...
;:Private:
;-
pro irf_mrd_hread, unit, header, status, SILENT = silent, FIRSTBLOCK = firstblock, $
...
;:Private:
;-			
PRO irf_mrd_fxpar, hdr, xten, nfld, nrow, rsize, fnames, fforms, scales, offsets
...
;
  ...
  xten = irf_FXpar(hdr, 'XTENSION')
  nfld = irf_FXpar(hdr, 'TFIELDS')
  nrow = long64(irf_FXpar(hdr, 'NAXIS2'))
  rsize = long64(irf_FXpar(hdr, 'NAXIS1'))
...
;
  ...
  IF ( n_mforms GT 0 ) THEN BEGIN
      ...
      igood = WHERE(irf_valid_num(numst,/INTEGER), n_mforms)
      IF n_mforms GT 0 THEN BEGIN
          mforms = mforms[igood]
  ...
  irf_match, keyword, fnames_names, mkey_names, mnames, count = N_mnames

  irf_match, keyword, scales_names, mkey_scales, mscales, count = N_mscales

  irf_match, keyword, offsets_names, mkey_offsets, moffsets,count = N_moffsets
...
;
                      ...
                      test2 = irf_gettok(test2,' ')
                      IF irf_valid_num(value,val1) && irf_valid_num(value2,val2) $
                        THEN BEGIN
...
NOT_COMPLEX:
                      ...
                      value = test
                      IF ~irf_valid_num(value) THEN GOTO, GOT_VALUE
...
;:Private:
;-
function  irf_mrd_dofn, name, index, use_colnum, alias=alias
compile_opt idl2, hidden
...
;:Private:
;-
pro irf_mrd_doff, form, dim, type 
compile_opt idl2, hidden 
...
;:Private:
;-
function irf_mrd_chkfn, name, namelist, index 
 compile_opt idl2, hidden
    ; 
    ; 
...
;:Private:
;-
function irf_mrd_unsigned_offset, type
compile_opt idl2, hidden
...
;:Private:
;-
function irf_mrd_chkunsigned, bitpix, scale, zero, unsigned=unsigned
compile_opt idl2, hidden
...
;:Private:
;-
function irf_mrd_unsignedtype, data
 compile_opt idl2, hidden      
 type = size(data,/type) 
...
;:Private:
;-
pro irf_mrd_atype, form, type, slen
compile_opt idl2, hidden
...
;:Private:
;-
pro irf_mrd_read_ascii, unit, range, nbytes, nrows, nfld, typarr, posarr,   $
...
compile_opt idl2, hidden
    ...
    if range[0] gt 0 then irf_MRD_skip, unit, nbytes*range[0]
    readu,unit, bigstr
    ...
    irf_MRD_skip, unit, nskipRow*nbytes+nskipB
...
;:Private:
;-
pro irf_mrd_ascii, header, structyp, use_colnum,   $
    range, table, $
...
; Set default 'null' values   
    ...
    if strmid(irf_FXpar(header, 'XTENSION'),0,8) ne 'TABLE   ' then begin
        message, 'ERROR - Header is not from ASCII table.',/CON
        status = -1;
        return
    endif

    nfld = irf_FXpar(header, 'TFIELDS')
    nrows = long64( irf_FXpar(header, 'NAXIS2'))
    nbytes = long64( irf_FXpar(header, 'NAXIS1'))
    ...
    for i=0, nfld-1 do begin
        ...
        fname = irf_FXpar(header, 'TTYPE' + suffix, count=cnt)
	tname[i] = fname
	if cnt eq 0 then xx = temporary(fname)
        fform = irf_FXpar(header, 'TFORM' + suffix)
        fpos = irf_FXpar(header, 'TBCOL' + suffix)
        fnull = irf_FXpar(header, 'TNULL' + suffix, count=cnt)
	if cnt eq 0 then fnull = ''
        scales[i] = irf_FXpar(header, 'TSCAL' + suffix)
        if scales[i] eq 0.0d0 then scales[i] = 1.0d0
        offsets[i] = irf_FXpar(header, 'TZERO'+suffix)
        
        fname = strupcase( irf_MRD_dofn(fname,i+1, use_colnum, alias=alias))

        if i GT 0 then fname = irf_MRD_chkfn(fname, fnames, i) ;Check for duplicates
	fnames[i] = fname
        
        irf_MRD_atype, fform, ftype, flen
        typarr[i] = ftype
        lenarr[i] = flen
        posarr[i] = fpos
        nullarr[i] = fnull
    ...
    if ~scaling && ~keyword_set(columns) then begin
        table = irf_MRD_struct(fnames, fvalues, nrows, structyp=structyp, $
           silent=silent)
    endif else begin
        table = irf_MRD_struct(fnames, fvalues, nrows, silent=silent)
    endelse
...
;:Private:
;-
pro irf_mrd_columns, table, columns, fnames, fvalues, $
...
compile_opt idl2, hidden
    ...
    if type eq 7 then begin
       irf_match, strupcase(columns), strupcase(fnames), tmp, tcols,count=nmatch 
    ...
    if n_elements(tcols) le 0 then begin
        ...
        if scaling || n_elements(vcls) gt 0 then begin
            tabx = irf_MRD_struct(fnames, fvalues, ndim, silent=silent )
        endif else begin
            tabx = irf_MRD_struct(fnames, fvalues, ndim, structyp=structyp, silent=silent )
        endelse
...
;:Private:
;-
pro irf_mrd_read_image, unit, range, maxd, rsize, table, rows = rows,status=status, $
     unixpipe = unixpipe
 compile_opt idl2, hidden
    ; 
    ; Unit          Unit to read data from. 
    ...
    if range[0] gt 0 then irf_MRD_skip, unit, range[0]*rsize
    ...
    irf_MRD_skip, unit, skipB
    if unixpipe then swap_endian_inplace, table,/swap_if_little

    ; Fix offset for unsigned data
    type = irf_MRD_unsignedtype(table)
    if type gt 0 then $
	table = table - irf_MRD_unsigned_offset(type)
...
;:Private:
;-
pro irf_mrd_axes_trunc,naxis, dims, silent
compile_opt idl2, hidden
    mysilent = silent
...
;:Private:
;-
pro irf_mrd_image, header, range, maxd, rsize, table, scales, offsets, scaling, $
 ...
 compile_opt idl2, hidden
    ...
    bitpix= irf_FXpar(header, 'BITPIX') 
    if naxis gt 0 then begin 
          dims = long64(irf_FXpar(header, 'NAXIS*', Count = N_axis)) 
...
; Check if extra NAXISn keywords are present (though this is not legal FITS)
    ...
    gcount = irf_FXpar(header, 'GCOUNT') 
    pcount = irf_FXpar(header, 'PCOUNT')
    isgroup = irf_FXpar(header, 'GROUPS')
    gcount = long(gcount)

    xscale = irf_FXpar(header, 'BSCALE', count=cnt)
    if cnt eq 0 then xscale = 1      ;Corrected 06/29/06
    
    xunsigned = irf_MRD_chkunsigned(bitpix,  xscale, $
				irf_FXpar(header, 'BZERO'), unsigned=unsigned)
	...
	maxd = gcount
        ...
        irf_MRD_axes_trunc, naxis, dims, keyword_set(silent)
        ...
        table = irf_MRD_struct(['params','array'], values, range[1]-range[0]+1, $
                           silent=silent)

	if xunsigned then begin
	    irf_fxaddpar,header, 'BZERO', 0, 'Reset by MRDFITS v'+irf_MRD_version()
	endif
        ...
        for i=0, pcount-1 do begin
            ...
            scales[i] = irf_FXpar(header, 'PSCAL'+istr)
            if scales[i] eq 0.0d0 then scales[i] =1.0d0
	    
            offsets[i] = irf_FXpar(header, 'PZERO'+istr)
	    
            scales[pcount] = irf_FXpar(header, 'BSCALE')
            if scales[pcount] eq 0.0d0 then scales[pcount] = 1.0d0
            offsets[pcount] = irf_FXpar(header, 'BZERO')
	...
	if xunsigned then begin
	    irf_fxaddpar,header, 'BZERO', 0, 'Updated by MRDFITS v'+irf_MRD_version()
	endif
	
        scales[0] = irf_FXpar(header, 'BSCALE')
        offsets[0] = irf_FXpar(header, 'BZERO')
...
;:Private:
;-
pro irf_mrd_ptrscale, array, scale, offset
compile_opt idl2, hidden
...
;:Private:
;-
pro irf_mrd_string, table, header, typarr, $
...
; because scaling will change the tag data types.
        ...
        tabx = irf_MRD_struct(fnames, fvalues, nrec, structyp=structyp, silent=silent )
...
;:Private:
;-
pro irf_mrd_scale, type, scales, offsets, table, header,  $
...
; Now copy the scaled items indexed by w after applying the scaling.        
		...
		if dtype eq 10 then $
		    irf_MRD_ptrscale, table.(w[i]), scales[w[i]], offsets[w[i]]
		
                tabx.(w[i]) = table.(w[i])*scales[w[i]] + offsets[w[i]]
		
            istr = strtrim(w[i]+1,2)
            irf_fxaddpar, header, 'TSCAL'+istr, 1.0, ' Set by MRD_SCALE'
            irf_fxaddpar, header, 'TZERO'+istr, 0.0, ' Set by MRD_SCALE'
    ...
    ; Now process images and random groups.
        ...
        if sz[sz[0]+1] ne 8 then begin
            ...
            endelse
            irf_fxaddpar, header, 'BSCALE', 1.0, 'Set by MRD_SCALE'
            irf_fxaddpar, header, 'BZERO', 0.0, 'Set by MRD_SCALE'
            ...
            s2 = s2+')'
            tabx = irf_MRD_struct(['params', 'array'],[s1,s2],ngr, silent=silent)

            for i=0, nparam-1 do begin
                istr = strcompress(string(i+1),/remo)
                irf_fxaddpar, header, 'PSCAL'+istr, 1.0, 'Added by MRD_SCALE'
                irf_fxaddpar, header, 'PZERO'+istr, 0.0, 'Added by MRD_SCALE'
                tabx.(0)[i] = table.(0)[i]*scales[i]+offsets[i]
            endfor
	    
            tabx.(1) = table.(1)*scales[nparam] + offsets[nparam]
            irf_fxaddpar, header, 'BSCALE', 1.0, 'Added by MRD_SCALE'
            irf_fxaddpar, header, 'BZERO', 0.0, 'Added by MRD_SCALE'
            table = temporary(tabx)
        endelse
    endelse
...
;:Private:
;-
pro irf_mrd_varcolumn, vtype, array, heap, off, siz
compile_opt idl2, hidden
    ...
    unsigned = irf_MRD_unsigned_offset(unsigned)
...
;:Private:
;-
pro irf_mrd_fixcolumn, vtype, array, heap, off, siz
compile_opt idl2, hidden
...
;:Private:
;-
pro irf_mrd_read_heap, unit, header, range, fnames, fvalues, vcls, vtpes, table, $ 
...
compile_opt idl2, hidden
    ...
    heapoff = long64(irf_FXpar(header, 'THEAP')) 
    sz = irf_FXpar(header, 'NAXIS1')*irf_FXpar(header, 'NAXIS2')
    ...
    if (heapoff > sz) then begin
        irf_MRD_skip, unit, heapoff-sz
    endif
 
    ; Get the size of the heap. 
    pc = long64(irf_FXpar(header, 'PCOUNT')) 
    ...
    if ~scaling && ~keyword_set(columns) then begin
        tablex = irf_MRD_struct(fnames, fvalues, n_elements(table), structyp=structyp, $
                            silent=silent)
    endif else begin
        tablex = irf_MRD_struct(fnames, fvalues, n_elements(table), silent=silent)
    endelse
    ...
    for i=0, n_elements(fnames)-1 do begin
		...
		if varying && keyword_set(pointer_var) && (vtype ne 'A') then begin
		    irf_MRD_varcolumn, vtype, curr_colx, heap, off, siz
		endif else begin
		    irf_MRD_fixcolumn, vtype, curr_colx, heap, off, siz
		endelse
...
;:Private:
;-
pro irf_mrd_read_table, unit, range, rsize, structyp, nrows, nfld, typarr, table, rows = rows, $
     unixpipe = unixpipe
compile_opt idl2, hidden 
    ; 
    ; 
    ...
    if range[0] gt 0 then irf_MRD_skip, unit, rsize*range[0]
    readu,unit, table
    ...
    for i=0, nfld-1 do begin
	    ...
	    type = irf_MRD_unsignedtype(table.(i))

	    if type gt 0 then begin	    
	        table.(i) = table.(i) - irf_MRD_unsigned_offset(type)
	    endif
...
;:Private:
;-
pro irf_mrd_tdim, header, index, flen_, arrstr, no_tdim=no_tdim
compile_opt idl2, hidden
    ...
    if flen_ eq 0 then flen=1 else flen=flen_ ;bug fix, handle case where a scalar is treated as a (possibly multidimensional) array with all dimensions of size 1
    arrstr = strcompress(string(flen),/remo)

    if keyword_set(no_tdim) then return

    tdstr = irf_FXpar(header, 'TDIM'+strcompress(string(index),/remo))
...
;:Private:
;-
pro irf_mrd_table, header, structyp, use_colnum,           $ 
...
; NEW WAY: E.S.S.
    ...
    irf_MRD_fxpar, header, xten, nfld, nrow, rsize, fnames, fforms, scales, offsets
...
;    rsize = irf_FXpar(header, 'NAXIS1') 
    ...
    for i=0, nfld-1 do begin
        ...
        fname = irf_MRD_dofn(fname, i+1, use_colnum, alias=alias)
	
        ;; check for a name conflict
        fname = irf_MRD_chkfn(fname, fnames2, i)
        ...
        if dim eq 1 then begin
            ...
            irf_MRD_tdim, header, i+1, dim, str, no_tdim=no_tdim
            dimfld[i] = str
        endelse
	    ...
	    xunsigned = irf_MRD_chkunsigned(bitpix[ppos], scales[i],       $
				       offsets[i], $
				       unsigned=unsigned)
        ...
        for j=0, n_elements(types) - 1 do begin
            ...
            if ftype eq types[j] then begin
                ...
                xunsigned = irf_MRD_chkunsigned(bitpix[j], scales[i], $
                                            offsets[i], $
                                            unsigned=unsigned)

		if xunsigned then begin		     
		    irf_fxaddpar, header, 'TZERO'+istr, 0, 'Modified by MRDFITS V'+irf_MRD_version()
  ...
  next_col: 
    ...
    if n_elements(vcls) eq 0  &&  (~scaling) && ~keyword_set(columns) then begin
        ...
        table = irf_MRD_struct(fnames, fvalues, nrow, structyp=structyp,  silent=silent )
	
    endif else begin
	
        table = irf_MRD_struct(fnames, fvalues, nrow, silent=silent )
...
;:Private:
;-
function irf_mrdfits, file, extension, header,      $
        structyp = structyp,                    $
    ...
    if keyword_set(version) then $
        print,'MRDFITS: Version '+irf_MRD_version() + 'April 29, 2011'
    ...
    if (dtype gt 0) && (dtype lt 4) then begin    ;File unit number specified
        ...
        if irf_FXmove(unit,extension) lt 0 then return, -1
    
    endif else begin                         ;File name specified
        unit = irf_FXposit(file, extension, compress=compress, unixpipe=unixpipe, $
    ...
    irf_MRD_hread, unit, header, status, SILENT = silent, ERRMSG = errmsg
...
; file using a pipe.
    ...
    if ~keyword_set(no_fpack) then $
         if (inputunit EQ 0) && (~unixpipe) then begin 
            if irf_sxpar(header,'ZIMAGE') then begin 
	    free_lun,unit
            unit = irf_FXposit(file, extension, compress=compress, /fpack, $ 
    ...
    xten = strtrim( irf_FXpar(header,'XTENSION'), 2)
    ...
    if type eq 0 then begin
        ...
        irf_MRD_image, header, arange, maxd, rsize, table, scales, offsets, $
          scaling, status, silent=silent, unsigned=unsigned, $
           rows= rows
       if (status ge 0) && (rsize gt 0) then begin
           irf_MRD_read_image, unit, arange, maxd, rsize, table, rows = rows,$
       ...
       size = rsize
        ...
        irf_MRD_ascii, header, structyp, use_colnum,                              $
        ...
        if (status ge 0)   &&  (size gt 0)  then begin
            ...
            ;*** Read data.
            irf_MRD_read_ascii, unit,  arange, nbytes, nrows,   $
              nfld, typarr, posarr, lenarr, nullarr, table,  rows= rows
              
            ;*** Extract desired columns.
            if (status ge 0) && keyword_set(columns) then                  $
                irf_MRD_columns, table, columns, fnames, fvalues, vcls, vtps, $
        ...
        irf_MRD_table, header, structyp, use_colnum,                            $
	  ...
	  emptystring=emptystring
        ...
        if (status ge 0)  &&  (size gt 0)  then begin
	      ...
	      irf_MRD_string, table, header, typarr, $
            ...
            if (status ge 0) && n_elements(vcls) gt 0 then begin 
                ...
                irf_MRD_read_heap, unit, header, arange, fnames, fvalues,             $
	        ...
	        sz = long64(irf_FXpar(header, 'NAXIS1'))* $
                     long64(irf_FXpar(header,'NAXIS2')) +  $
		       long64(irf_FXpar(header, 'PCOUNT'))
	        skipB = 2880 - sz mod 2880
	        if (skipB ne 2880) then irf_MRD_skip, unit, skipB
            endelse
...
; apply scalings.
	...
	noscale = array_equal(scales,1.d0) &&  array_equal(offsets,0.0) 
        ...
        if ~noscale then irf_MRD_scale, type, scales, offsets, table, header,  $
...
;:Private:       
;-
function irf_SXPAR, hdr, name, abort, COUNT=matches, COMMENT = comments, $
...
;  value of the number field in NUMBER.
		...
		igood = where(irf_valid_num(numst,/INTEGER), matches)
		if matches GT 0 then begin 
		     nfound = nfound[igood]
...
;:Private:
;-
PRO irf_FXADDPAR_CONTPAR, VALUE, CONTINUED
...
;:Private:
;-
PRO irf_FXADDPAR_CONTWARN, HEADER, NAME
...
;  string encoded with the CONTINUE convention.
  ...
  CONTKEY = irf_FXPAR(HEADER, 'LONGSTRN', COUNT = N_LONGSTRN)
...
;  clean now.
  ...
  irf_FXADDPAR, HEADER, 'LONGSTRN', 'OGIP 1.0', $
    ' The OGIP long string convention may be used.', $
    BEFORE=NAME

  irf_FXADDPAR, HEADER, 'COMMENT', $
...
;:Private:
;-
FUNCTION irf_FXPARPOS, KEYWRD, IEND, BEFORE=BEFORE, AFTER=AFTER
;
...
;:Private:
;-
PRO irf_FXADDPAR, HEADER, NAME, VALUE, COMMENT, BEFORE=BEFORE,      $
...
;
                END ELSE IF NN EQ 'COMMENT ' THEN BEGIN
                        I = irf_FXPARPOS(KEYWRD,IEND,AFTER=AFTER,BEFORE=BEFORE)
                        IF I EQ IEND THEN I =   $
                            irf_FXPARPOS(KEYWRD,IEND,AFTER='COMMENT',$
                                     BEFORE='HISTORY')
...
;
                END ELSE BEGIN
                        I = irf_FXPARPOS(KEYWRD,IEND,AFTER=AFTER,BEFORE=BEFORE)
                        IF I EQ IEND THEN I =   $
                            irf_FXPARPOS(KEYWRD,IEND,AFTER='',BEFORE='COMMENT')<$
                            irf_FXPARPOS(KEYWRD,IEND,AFTER='',BEFORE='HISTORY')
                        HEADER[I+1] = HEADER[I:N-2]     ;move rest up
...
;
        IF KEYWRD[0] EQ 'XTENSION' THEN BEGIN
                XTEN = irf_FXPAR(HEADER,'XTENSION')
...
;
        I = irf_FXPARPOS(KEYWRD,IEND,AFTER=AFTER,BEFORE=BEFORE)
        IF I EQ IEND THEN I =                                     $
            irf_FXPARPOS(KEYWRD,IEND,AFTER=AFTER,BEFORE='')         < $
            irf_FXPARPOS(KEYWRD,IEND,AFTER=AFTER,BEFORE='COMMENT')  < $
            irf_FXPARPOS(KEYWRD,IEND,AFTER=AFTER,BEFORE='HISTORY')
;
...
;
                        IF NOT KEYWORD_SET(NOCONTINUE) THEN $
                             irf_FXADDPAR_CONTPAR, VAL, CVAL  ELSE $
...
;  more than one element if that is so.
                            ...
                            ;; warning to header
                               ...
                               irf_FXADDPAR_CONTWARN, HEADER, NAME
                            ENDIF
                            DONE_CONT:
                            RETURN
;
...
;:Private:
;-
FUNCTION irf_FXPAR, HDR, NAME, ABORT, COUNT=MATCHES, COMMENT=COMMENTS, $
        ...
        IF VECTOR THEN BEGIN
            ...
            IF ( MATCHES GT 0 ) THEN BEGIN
                ...
                FOR I = 0, MATCHES-1 DO         $
                    IF irf_valid_num( NUMST[I], NUM) THEN NUMBER[I] = NUM
    ...
    IF NOT KEYWORD_SET(NOCONTINUE) THEN BEGIN
                    ...
                    IF (STRLEN(VAL) GT 0) AND $
                      ...
                      (STRMID(HDR[NFOUND[I]+OFF],0,8) EQ 'CONTINUE') THEN BEGIN
                       IF (SIZE(irf_FXPAR(HDR, 'LONGSTRN',/NOCONTINUE)))[1] EQ 7 THEN BEGIN                    
                      VALUE = STRMID(VAL, 0, STRLEN(VAL)-1)
                      TEST = HDR[NFOUND[I]+OFF]
                      TEST = STRMID(TEST, 8, STRLEN(TEST)-8)
                      TEST = STRTRIM(TEST, 2)
...
;
                        IF STRLEN(TEST2) EQ 0 THEN GOTO, NOT_COMPLEX
                        VALUE2 = irf_gettok(TEST2,' ')
                        IF irf_valid_num(VALUE,VAL1) AND irf_valid_num(VALUE2,VAL2) $
                                THEN BEGIN
                            VALUE = COMPLEX(VAL1,VAL2)
                            GOTO, GOT_VALUE
                        ENDIF
...
NOT_COMPLEX:
                        ON_IOERROR, GOT_VALUE
                        VALUE = TEST
                        IF NOT irf_valid_num(VALUE) THEN GOTO, GOT_VALUE
...
;:Private:
;-
FUNCTION irf_DETABIFY, CHAR_STR
;
	ON_ERROR, 2
;
;  Check the number of parameters.
...
;:Private:
;-
function irf_gettok,st,char, exact=exact, notrim=notrim
;----------------------------------------------------------------------
  On_error,2                           ;Return to caller
  compile_opt idl2

   if N_params() LT 2 then begin
       print,'Syntax - token = irf_gettok( st, char, [ /EXACT, /NOTRIM] )'
       return,-1
   endif
...
; if char is a blank treat tabs as blanks
 ...
 if ~keyword_set(exact) then begin
    ...
    if char EQ ' ' then begin 
       tab = string(9b)                 
       if max(strpos(st,tab)) GE 0 then st = irf_repchr(st,tab,' ')
    endif
  endif
  token = st
...
;:Private:
;-
pro irf_match, a, b, suba, subb, COUNT = count, SORT = sort, epsilon=epsilon
;-------------------------------------------------------------------------
 On_error,2
 compile_opt idl2
...
;:Private:
;-
pro irf_remchar,st,char	;Remove character
 compile_opt idl2                             
 if N_params() LT 2 then begin
...
;:Private:
;-
function irf_strn, number, LENGTH = length, PADTYPE = padtype, PADCHAR = padchar, $
                       FORMAT = Format
 On_error,2
  if ( N_params() LT 1 ) then begin
...
;:Private:
;-
pro irf_textclose,textout=textout
;-----------------------------------------------------------
; CLOSE PROPER UNIT
;
...
;:Private:
;-
PRO irf_TEXTOPEN,PROGRAM,TEXTOUT=TEXTOUT, STDOUT = STDOUT, MORE_SET = more_set, $
...
;:Private:
;-            
FUNCTION irf_valid_num, string, value, INTEGER=integer
 On_error,2
 compile_opt idl2 
...
;:Private:
;-
function irf_mrd_struct, names, values, nrow, no_execute = no_execute,  $
...
;`
		...
		if strlen(value) EQ 1 then v= value else begin 
	        type = irf_gettok(value,'(')
		if type eq 'string' then $
			junk = irf_gettok(value,',')      ;remove "replicate(32b"
		dimen_string = irf_gettok(value,')')	
...
;
;-
function iuvs_read_fits,infname_,verbose=verbose,swap=swap,_extra=extra
; MODIFICATION HISTORY:
...
; $Id: eve_read_whole_fits.pro,v 3.3 2011/09/02 21:49:50 dlwoodra Exp $
  f=file_Search(infname_,count=count)
  if count eq 0 then message,'File not found: '+infname_
  infn = f[-1]
  compress=strlowcase((reverse(strsplit(infn,'.',/extract)))[0]) eq 'gz' 
  ; determine the number of HDUs to read
  irf_fits_info,silent=~keyword_set(verbose),infn,n_ext=n_hdus ; the only programmatic way to get number of HDUs?
  openr,inf,infn,/get_lun,compress=compress,/swap_if_little
  status=0
  hdu=0
  this_hdu=irf_mrdfits(inf,0,this_header,/unsigned,silent=~keyword_set(verbose),compress=compress,_extra=extra,status=status)
  while status ge 0 and hdu le n_hdus do begin
    if hdu eq 0 then begin
      result=create_struct('primary',this_hdu,'primary_header',this_header)
    endif else begin
      w=where(strmid(this_header,0,8) eq 'EXTNAME ',count)
      if count eq 1 then begin
        this_hdu_name=strtrim((strsplit(this_header[w],"'",/extract))[1])
      endif else begin
        this_hdu_name=string(format='(%"HDU%03d")',hdu)
      endelse
    ...
    if hdu le n_hdus then this_hdu=irf_MRDfits(inf,0,this_header,/unsigned,silent=~keyword_set(verbose),_extra=extra,status=status)
  endwhile
  free_lun,inf
