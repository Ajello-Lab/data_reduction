...
; Additionally, a csv file containing the calculated qualities
; of the data will be outputted
;
pro ajello_lab_calibration_fuv_n2_20260513
  ...
  case (get_login_info()).user_name of
    'holsclaw': begin
      file_path = '/Volumes/projects/Phase_Development/MAVEN/IUVS_Data/IUVS_Breadboard/Round_12/data_reduction/'
  ...
  qualities = fltarr(n_elements(files))
  num_light = fltarr(n_elements(files))
  ...
  for j = 0, n_elements(files) - 1 do begin
    ...
    sObj.restore, ['arr', 'Temp_Light']
    arr = float(arr)
    num_light[j] = n_elements(temp_light)
    obj_destroy, sObj
    ...
    ;
    win = window(dim = [1400, 600], buffer = 1)
    xr = [110, 190]
    ...
    ; calculates quality quanity and stores in an array
    total = total(spec) ; assumes that spec length is constant amount measurement data
    qualities[j] = alog10(total * num_light[j])
  endfor

  csv_name = path_save + 'Computed_Qualities.csv'
  write_csv, csv_name, files, qualities, num_light, header = ['files', 'qualities', 'Num_light']
