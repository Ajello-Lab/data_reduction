;+
; NAME: 
; AJELLO_LAB_CALIBRATE
; 
; PURPOSE:
; This routine will calibrate the reduced data contained in an IDL save file, 
;   and place the results in a new IDL save file.
; 
; INPUTS:
; fsave_data: Full path to the existing IDL save file containing the processed dataset.
; 
; OUTPUTS:
; fsave_cal: Full path to the generated IDL save file containing the calibrated data.
;   
; KEYWORDS:
; yr: (optional) Specifies the starting and ending spatial row of the narrow slit (default is 150:900)
; path_cal: (optional) Destination path of IDL save file containing calibrated data.
;-
pro ajello_lab_calibrate, fsave_data, fsave_cal, yr=yr_spec, path_cal=path_cal

if n_params() eq 0 then begin
  fsave_data = '/Users/holsclaw/MAVEN/Ajello_lab/_Big_e-gun_roundIII/data_reduction/H2_100eV_FUV_test_1_image1.idl'
...
;ajello_lab_outlier_identify, arr, channel, mask, frac=frac ;, x_limits=x_limits, show_plot=show_plot
;ajello_lab_outlier_interpolate, arr, mask, arri
;img1 = image( arr )
;img2 = image( arri )
;img3 = image( mask, max_value=1)
;img4 = image( arr_dark_sdv, min_value=200., max_value=500 )
;h = histogram(arr_Dark_sdv,locations=xh)
;p = plot( xh, h, /ylog )
