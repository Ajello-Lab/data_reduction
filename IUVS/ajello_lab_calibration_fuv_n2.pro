...
;  endif
  ...
  for i = 0, n_peaks - 1 do begin
    ...
    if count_data gt 0 then begin
      pos_cen_data_pix[i] = centroid( pix_vec[ndx_speci], spec[ndx_speci] )
      pos_cen_data_wave[i] = centroid( wl_spec[ndx_speci], spec[ndx_speci] )
      pos_cen_ref_wave[i] = centroid( wl_ref[ndx_refi], ref[ndx_refi] )
